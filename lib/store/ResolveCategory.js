...
class ResolveCategory {
  ...
  static parseSummary(docPath, docBaseName) {
    stack = [];
    nextStack = [];
    depth = 0;
    ...
    Output.getInstance().outputManifest(docBaseName, res.accumulate);
  }
  ...
  static refineManifestAfterResolvePostMeta(docPath, docBaseName, postmeta) {
    const path = `${Output.getInstance().outputPath}/${docBaseName}/manifest.js`;
